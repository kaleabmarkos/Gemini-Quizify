...
class DocumentProcessor:
    """
    This class encapsulates the functionality for processing uploaded PDF documents using Streamlit(Make it take only PDF Files and multiple files )
    and Langchain's PyPDFLoader(To Extact the uploaded files, then clean up after that proccess). It provides a method to render a file uploader widget, process the
    ...
    def ingest_documents(self):
        ...
        1. Utilize the Streamlit file uploader widget to allow users to upload PDF files.
           From st.file_uploader() with the 'type' parameter set to 'pdf'.
        2. For each uploaded PDF file:
           ...
           b. Using Langchain's PyPDFLoader on the path of the temporary file to extract pages.
        ...
        if uploaded_files is not None:
            for uploaded_file in uploaded_files:
                ...
                # Generating a unique file name with the original name preserved
                temp_file_name = f"{original_name}_{unique_id}{file_extension}"
